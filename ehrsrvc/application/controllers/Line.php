...
class Line extends CI_Controller{
    public function __construct() {
		...
		$this->load->model("Line_model", "line", TRUE);
    ...
    public function insertIntoLine()
    {
        ...
        if($client_token->jti==$server_token ){
            ...
            $register = $this->line->insertIntoLine($request,$hospital_id);
			if($register){
				$json_response = [
    ...
    public function updateLine()
    {
        ...
        if($client_token->jti==$server_token ){
            ...
            $register = $this->line->updateLine($request,$hospital_id);
			if($register){
				$json_response = [
                    "msg_status"=>HTTP_SUCCESS,
                    "msg_data"=>"SUCCESS",
    ...
    public function getLineList()
    {
        ...
        if ($client_token != "") {
            if ($client_token->jti == $server_token) {
                ...
                $resultdata = $this->line->getLineListByHospital($hospital_id);
    ...
    public function getLineDataById()
    {
        CUSTOMHEADER::getCustomHeader();
        $json_response = [];
        $headers = $this->input->request_headers();
        ...
        if ($client_token != "") {
            if ($client_token->jti == $server_token) {
               ...
               $where = array('line_master.id' =>$challan_id );

                $resultdata = $this->commondatamodel->getSingleRowByWhereCls('line_master',$where);
